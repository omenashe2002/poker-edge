...
| Tab | What it does |
|---|---|
| 📖 **Study** | A **26-lesson course in 5 modules** (foundations → preflop architecture → the math layer → postflop → the human game), each lesson with learning objectives, first-principles explanations, key takeaways, and a retrieval-practice checkpoint quiz. Every lesson links **curated videos from the masters** (GTO Wizard, Bart Hanson, Jonathan Little, Polk, Negreanu, Galfond…) via stale-proof links, plus a full video library. A **65-term glossary** — every jargon word (RFI, mixed, MDF…) auto-links inside lessons and opens a definition sheet on tap. 55 preflop charts — including hero limp strategy (SB limp-mix, over-limping) and vs-limpers isos — with **per-hand play frequencies** (25/50/75/100%) shown as proportional cell shading, tap-any-cell explanations, and auto-generated border summaries. A dedicated **MTT · WPT reference**: cash-vs-tournament core differences plus a stage-by-stage ClubWPT Gold daily-freeroll playbook. Plus the live cheat sheet and player-type field guide. |
...
| ♠️ **Live** | Start a session, lay out the table, tap seats to mark who played/raised each hand (auto VPIP/PFR), tag player types, take notes. The exploit advisor shows the max-EV adjustments vs each type. Players persist across sessions — your private book of regulars. Quick 10-second hand capture between hands. Plus a **Home Game ledger**: buy-ins/re-buys/cash-outs per player, chip-mismatch detection, and minimal who-pays-whom settlement (at most n-1 payments) with a copyable summary. |
| 🃏 **Hands** | Hand logger v2: effective stacks (bb), position, preflop line, three streets of action, boards, notes — and an **EDGE recommendation engine** that reads each logged hand (stack depth, line, result, villain type) and tells you what to do differently next time. GTO preflop check + leak tagging included. |
